function [index_left, index_right, which_feature, threshold_feature, DD, IN, indexX] = split_cmrf(X, Y, index, V_inv, mtree,NN,copula,Command, Alpha, min_leaf)
...
for j = 1:length(f_1)
    ...
    %for k1 = min_leaf:min_leaf:(N-min_leaf) % calculate D(N)
        %k=k1/min_leaf;
        %yk_left = y(idx(1:k1),:);
